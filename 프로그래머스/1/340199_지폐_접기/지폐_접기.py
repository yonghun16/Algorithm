...
# 📥 Input
def get_input_data() -> tuple[list[str], list[str]]:
    wallet: list[str] = input().split()
    bill: list[str] = input().split()

    return wallet, bill


# ⚙️ Logic
def solution(wallet: list[str], bill: list[str]) -> int:
    answer: int = 0
    wx: int = int(wallet[0])
    wy: int = int(wallet[1])
    bx: int = int(bill[0])
    ...
    # 지갑도 정렬: 긴 변, 짧은 변 순서로 통일
    if wy > wx:
        wx, wy = wy, wx
    if by > bx:
        bx, by = by, bx

    while wx < bx or wy < by:
        answer += 1
        bx //= 2
        if wx < bx or wy < by:
            if by > bx:
