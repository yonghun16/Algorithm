    ...
    for oj in OJ_LIST:
        ...
        for folder in os.listdir(oj_path):
            ...
            if parsed and parsed[0].lower() == oj.lower():
                ...
                all_problems.append(
                    (parsed[0], parsed[1], parsed[2], level, tags, link, mtime)
                )
...
def write_readme(path, title, problems_by_oj):
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("""| 백준 (Solved.ac) | 프로그래머스 (Programmers) |
        | :---: | :---: |
        ...
        for oj, problems in problems_by_oj.items():
            ...
            for oj, number, title, level, tags, link, _ in problems:
                ...
                f.write(
                    f"| {oj} | {number} | [{visible_title}]({link}) | {level} | {tags} | [코드](https://github.com/yonghun16/Algorithm/tree/main/Online_Judge_Problems/{oj}/{oj}_{number}_{encoded_title}) |\n"
                )
            f.write("\n")
...
def main():
    ...
    for oj in problems_by_oj:
        problems_by_oj[oj] = sorted(
            problems_by_oj[oj], key=lambda x: x[6], reverse=True
        )[:MAX_COUNT]
